...
for theta = 0 : 1/180 : 1
    ...
    i = i + 1;
    polarplot (table(:,1)*pi/180, table (:,2), "r");
    title('Ploarplot Map of the Environment');
    thetalim([0 180]);
    grid on;
end
clf()
...
for theta = 1 : -1/180 : 0
    ...
    title('Polarplot Map of the Environment');
    thetalim([0 180]);
    grid on;
end
...
figure(2)
plot (table(:,1)*pi/180, table (:,2),"g");
title('Average Plot of the Environment');
thetalim([0 180]);
grid on;
